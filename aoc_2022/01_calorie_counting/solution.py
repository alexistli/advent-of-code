...
def part1(data: list[str]) -> int:
    ...
    max_elf_calories = max([sum(elf_calories) for elf_calories in calories_grouped_by_elf])
...
def part2(data: list[str]) -> int:
    """Solve part 2."""
    calories_grouped_by_elf = get_calories_by_elf(data)
    summed_elf_calories = [sum(elf_calories) for elf_calories in calories_grouped_by_elf]
    top_3_elf_calories = sorted(summed_elf_calories, reverse=True)[:3]
